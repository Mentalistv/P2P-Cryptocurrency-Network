...
void printBlockMineTimes() {
    for (auto itr: nodes[0]->blocks) {
        printf("Block %d was mined at %f\n", itr.first, itr.second.timestamp);
    }
}
...
double printRatioForNumberOfBlocksGeneratedByCPUType() {
    ...
    double lowCPURatio = (double)lowCPU/(double) NUMBER_OF_NODES;
    double highCPURatio = 1 - lowCPURatio;

    double slowRatio = (double) slow / (double) NUMBER_OF_NODES;
    ...
    outfile << "Ratio of nodes generated by: Low CPU = " << lowCPURatio << " High CPU = " <<  highCPURatio << endl;
    
    outfile << "Ratio of nodes generated by: Slow Nodes = " << slowRatio << " Fast Nodes = " <<  fastRatio << endl;
}
...
int main(int argc, char const *argv[]) {    
    ...
    for (Node* node: nodes) {
        ...
        Block new_block = Block(BLOCK_ID_GENERATOR++, node->id, GENESIS_BLOCK_ID, 0.0 + delay, {});
        event_queue.push(new MineBlock(0.0 + delay, MINE_BLOCK, new_block));
    }
